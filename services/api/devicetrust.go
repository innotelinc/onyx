...
func loadDeviceTrustConfig() *deviceTrustConfig {
	...
	// CERULEAN_API_TOKEN may be a `vault://<mount>/<path>#<key>` reference
	// instead of a value. Resolve it eagerly so a broken reference surfaces at
	// startup, not on the first fleet request — and so `.env` can hold a
	// reference without a code change.
	if _, isRef := vault.ParseRef(cfg.token); isRef {
		vclient := vault.New(vault.ConfigFromEnv())
		resolved, err := vclient.ResolveEnv(context.Background(), cfg.token)
		if err != nil {
