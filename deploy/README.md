...
paths on every boot, and — the entry point — the script installer in
[`scripts/onyx-install`](../scripts/onyx-install).

## Layout

```
deploy/
├── systemd/onyx-*.service   # 5 daemons + the onyx CLI gateway
...
`Requires=`/`After=` chain the startup order privd → storaged+shared → core →
api; every daemon has `Restart=always` so transient failures self-heal. All
non-privd units run with `NoNewPrivileges` + strict-ish sandboxing; the
